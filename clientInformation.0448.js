...
function toggleMgmtAdv(lid, m) {
  ...
  if (h != null) {
    ...
    if (null != document.forms[0].elements["SPSSBWFEHC"])
      document.forms[0].elements["SPSSBWFEHC"].value = "Advanced";
  }
...
function OnToggleAllGroups(lid, cid, eid, te, tc) {
  ...
  if (null != document.forms[0].elements[_ppsi(lid, "spssECAH")]) {
    var bGCE = !(
      document.forms[0].elements[_ppsi(lid, "spssECAH")].value == "true"
    );
...
function GoSearch(
  ...
  if (k == "" || ui == "0") {
    alert("Ø§ÙØ±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù ÙÙÙØ© Ø¨Ø­Ø« ÙØ§Ø­Ø¯Ø© Ø£Ù Ø£ÙØ«Ø±.");
    if (null != event) {
...
function SetCrawlLogFilters(
  ...
  if (null != MinDtId) {
    ...
    if (elmDate != null && elmDate.value != null && elmDate.value != "") {
      ...
      var elmHours = document.getElementById(
        g_strDateTimeControlIDs[MinDtId] + "Hours"
      );
      if (elmHours != null) min += " " + elmHours.selectedIndex + ":";
      else min += " 00:";
      var elmMinutes = document.getElementById(
        g_strDateTimeControlIDs[MinDtId] + "Minutes"
      );
      if (elmMinutes != null) min += elmMinutes.selectedIndex * 5 + ":00";
      else min += ":00:00";
  ...
  if (null != MaxDtId) {
    ...
    if (
      elmMaxDate != null &&
      elmMaxDate.value != null &&
      elmMaxDate.value != ""
    ) {
      var max = elmMaxDate.value;
      var elmMaxHours = document.getElementById(
        g_strDateTimeControlIDs[MaxDtId] + "Hours"
      );
      if (elmMaxHours != null) max += " " + elmMaxHours.selectedIndex + ":";
      else max += " 00:";
      var elmMaxMinutes = document.getElementById(
        g_strDateTimeControlIDs[MaxDtId] + "Minutes"
      );
      if (elmMaxMinutes != null) max += elmMaxMinutes.selectedIndex * 5 + ":00";
      else max += ":00:00";
  ...
  if (sch == Url) {
    alert("Ø§ÙØ±Ø¬Ø§Ø¡ Ø§Ø®ØªÙØ§Ø± Ø£Ø­Ø¯ Ø¹ÙØ§ÙÙ Ø§ÙØªØµÙÙØ©.");
    if (null != event) {
      event.returnValue = false;
      return false;
    } else return;
...
/*
page: http://www.petroleum.gov.eg/
url: http://www.petroleum.gov.eg/_layouts/1025/search.js?rev=Fj9CLFcXvCRPvtP%2Bb%2FTL%2FA%3D%3D
*/
