        ...
        if (iCode <= iFFFF) {
          ...
          if (iCode <= i1FFFFF) {
            ...
            if (iCode <= i3FFFFFF) {
              ...
              strEncode +=
                "%" + parseInt(tempBin, 2).toString(16).toUpperCase();
...
function OnPTP(lid) {
  ULShpi:;
  var f = document.forms[0];
  if (null != f.elements[_ppsi(lid, "spssWFEH")])
    f.elements[_ppsi(lid, "spssWFEH")].value = "PageToPrevious";
  f.submit();
}
...
function OnPTN(lid) {
  ...
  var f = document.forms[0];
  if (null != f.elements[_ppsi(lid, "spssWFEH")])
    f.elements[_ppsi(lid, "spssWFEH")].value = "PageToNext";
  f.submit();
}
...
function OnPTT(a, event, lid, isIE) {
  ...
  if (isIE) {
    ...
    if (kCode == "\n" || kCode == "\r") {
      if (null != f.elements[_ppsi(lid, "spssWFEH")])
        f.elements[_ppsi(lid, "spssWFEH")].value = "DirectPageTo";
      if (null != f.elements[_ppsi(lid, "spssAPNH")])
        f.elements[_ppsi(lid, "spssAPNH")].value = a.value;
      f.submit();
    }
  } else {
...
function OnGroupBy(lid, strURI) {
  ...
  var f = document.forms[0];
  if (null != f.elements[_ppsi(lid, "spssWFEH")])
    f.elements[_ppsi(lid, "spssWFEH")].value = "GroupBy";

  if (null != f.elements[_ppsi(lid, "spssGBKH")])
    f.elements[_ppsi(lid, "spssGBKH")].value = strURI;
  f.submit();
}
...
function OnChangeSortBySelection(lid, cid) {
  ...
  var s = "";
  if (null != document.forms[0].elements[cid])
    s = document.forms[0].elements[cid].value;
...
function OnCCT(lid, uri, o) {
  ...
  if (null != document.forms[0].elements[_ppsi(lid, "spssWFEH")])
    document.forms[0].elements[_ppsi(lid, "spssWFEH")].value = "SortBy";
  if (null != document.forms[0].elements[_ppsi(lid, "SBCH")])
    document.forms[0].elements[_ppsi(lid, "SBCH")].value = "1";
...
function OnGFL(lid, s, t) {
  ...
  if (null != document.forms[0].elements[_ppsi(lid, "spssWFEH")])
    document.forms[0].elements[_ppsi(lid, "spssWFEH")].value =
      "SeeFullListLink";
...
function OnExpandCollapseAll(lid, cid, expand) {
  ...
  for (i = 0; i < l; i++) {
    ...
    if (null != sMatch.exec(tmp.id)) {
      ngid = RegExp.$1;
      if (ngid != gid) {
        fid = _ppsi(lid, "_tc_" + RegExp.$1);
        flk = document.links.item(fid);
        exid = _ppsi(lid, "_te_" + RegExp.$1);
        exlk = document.links.item(exid);
...
function GetTaxonomyApplyFilterUrl(taggingContainerId, columnId) {
  ULShpi:;
  var taggingContainer = document.getElementById(
    "TaggingControl_" + taggingContainerId
  );
  if (taggingContainer == null) {
    return "";
  }
  var taggingControl = new Microsoft.SharePoint.Taxonomy.ControlObject(
    taggingContainer
  );
  ...
  var terms = new Microsoft.SharePoint.Taxonomy.Term.getTerms(
    taggingControl.getRawText()
  );
  if (terms && terms.length > 0) {
    var url = document.URL.replace(/&start1=[0-9]+/gi, "");
...
function RenderTaggingControl(
  ...
  var taggingControl = document.getElementById(
    "TaggingControl_" + taggingContainerId
  );
  ...
  if (taggingControl.childNodes.length <= 1) {
    taggingControl["InputFieldId"] =
      "MetadataHiddenInput_" + taggingContainerId;
    if (isTagsColumn)
      taggingControl["TermSetId"] = "00000000-0000-0000-0000-000000000000";
    else taggingControl["TermSetId"] = termSetList;
  ...
  var moreLink = document.getElementById(
    morePanelSuffix + "_" + taggingContainerId
  );
  if (moreLink != null) {
...
function GoSearch(
  ...
  if (null != HdQId) {
    ...
    if (bApQ) sch += canonicalizedUtf8FromUnicode(" " + sa);
    else sch += "&a=" + canonicalizedUtf8FromUnicode(" " + sa);
  }

  var d = null,
    s = "",
    selVal = "",
    implicitContextualScoping = null != implicitScopeVal;
...
function ToggleRefMoreLessFilters(currElmt, showMore) {
  ...
  if (showMore) {
    ...
    currElmt.previousSibling.previousSibling.previousSibling.style.display =
      "block";
    currElmt.previousSibling.style.display = "none";
    currElmt.style.display = "none";
...
AjaxReplacementRequest.prototype = {
  ...
  set_testurl: function (url) {
    ULShpi:;
    this._testImageUrl = url;
  },
  add_completed: function (callbackFunction) {
    ULShpi:;
  ...
  invoke: function () {
    ...
    if (this._testImageUrl) {
      ...
      imgEl.alt = "";
      imgEl.onload = Function.createDelegate(
        this,
        this.invokeAfterTestImageLoad
      );
      imgEl.onerror = Function.createDelegate(
        this,
        this.invokeOnErrorImageLoad
      );
  ...
  onCompletedHandler: function (serverData) {
    ULShpi:;
...
FST_FlickScrollWindow.prototype = {
  ...
  myEndDrag: function () {
    ...
    if (this._isDragging) {
      ...
      if (
        Math.abs(this._lastSpeedX) > 0.2 ||
        Math.abs(this._lastSpeedY) > 0.2
      ) {
        this.startAnimation();
      }
    }
...
FST_ImageDisplayer.prototype = {
  ...
  display: function () {
    ...
    if (!this._withReached || !this._heightReached) {
      setTimeout(
...
function FST_ShowFullView() {
  ...
  theDiv.innerHTML =
    ...
    '<img class="srch-ext-fullViewImg" src="' +
    FST_fullViewImgSrc +
    '" onclick="FST_HideFullView();" />' +
    "</td></tr></table>" +
    "</center>";
}
...
function FST_CheckForPreview(
  ...
  if (document.getElementById(previewWindowId) != null) {
    ...
    if (!myFlickScrollWindow) {
      myFlickScrollWindow = new FST_FlickScrollWindow(
        previewWindowId,
        previewDivId,
        id
      );
...
function FST_ScrollRegister(id, dist, interval) {
  ...
  if (myScrollTimer) {
    ...
    if (dist < 0) delta = -5;
    setTimeout(
      "FST_ScrollRegister('" + id + "'," + (dist + delta) + ")",
      interval
    );
  }
...
function FST_ScrollStop(id, dist) {
  ...
  if (myScrollTimer) {
    ...
    if (myScrollTimer + 200 > now) FST_Scroll(id, dist);
...
function FST_ShowElement(elem, display) {
  ULShpi:;
  if (elem != null) {
...
function FST_StartDocPreviewFetch(conReq, totReq) {
  ULShpi:;
  gTotReq = totReq;
  for (var i = 0; i < conReq; i++) FST_DocPreviewFetch();
}
...
function FST_ThumbnailImageLoadError(id, tooltipfailed) {
  ...
  if (FST_ImageLoadError("FST_previewIcon_" + id) == 0) {
    ...
    var previewIconDivErrorElem = document.getElementById(
      "FST_prevIconDivError_" + id
    );
...
FST_WordDocument.prototype = {
  ...
  getPreviewImagesDoc: function (
    ...
    if (previewTooltip) this.previewTooltip = previewTooltip;
    if (previewTooltipFailed) this.previewTooltipFailed = previewTooltipFailed;
    ...
    myRequest.add_completed(
      Function.createDelegate(this, this.myOnCompleteDoc2)
    );
  ...
  DocPreviewFetchFailed: function () {
    ...
    var previewIconDivErrorElem = document.getElementById(
      "FST_prevIconDivError_" + this.docId
    );
    if (previewIconDivErrorElem != null && previewIconElem != null) {
      previewIconDivErrorElem.style.top = previewIconElem.height / 2 + "px";
      previewIconDivErrorElem.style.left = previewIconElem.width / 2 + "px";
...
FST_PPTPresentation.prototype = {
  ...
  getPreviewImagesPpt: function (
    ...
    myRequest.set_testurl(spsiteurl + "/_vti_bin/wacproxy.ashx?testImg=1");
    myRequest.add_completed(
      Function.createDelegate(this, this.myOnCompletePpt2)
    );
    this.ajaxRequest = myRequest;
    FST_RegisterDocPreviewFetch(myRequest);
  },
    ...
    if (result.indexOf("ERROR:") != 0) {
      ...
      for (var i = 0; i < presInf.images.length; i++) {
        ...
        if (i < presInf.images.length - 1) {
          html +=
            '<img class="srch-ext-previewDivPptImg"' +
            ' src="' +
            imgSrc +
            '" ' +
            ' id="FST_PrevImg_' +
          ...
          html +=
            '<img class="srch-ext-previewDivPptImgLast"' +
            ' src="' +
            imgSrc +
            '" ' +
            ' id="FST_PrevImg_' +
...
function PrepareForPostBack(target, userName, recentTabLabel) {
  ...
  var results = GetElementByIdSuffix(
...
/*
page: http://www.flynashville.com/
url: https://www.flynashville.com/_layouts/15/search.js?rev=xHY6V8aj58Nk0E2spHpElg%3D%3D
*/
