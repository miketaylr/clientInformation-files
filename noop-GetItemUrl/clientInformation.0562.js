...
function GoSearch(q, G, p, E, r, F, D, C, B, j, n, z, y, x, w, A, l, v) {
  ...
  var c = "?";
  if (suggestedQuery) c += "sq=1&";
  c += "k=" + encodeURIComponent(i);
  for (
    var k = [
        "rm",
        "rm1",
    ...
    if (m && m.length > 0) c += "&" + k[h] + "=" + m;
  }
  if (l != null && l != "") c += "&r=" + encodeURIComponent(l);
  if (null != p) {
    var t = document.forms[0].elements[p].value;
    if (E) c += canonicalizedUtf8FromUnicode(" " + t);
    else c += "&a=" + canonicalizedUtf8FromUnicode(" " + t);
  }
  var a = null,
    b = "",
    d = "",
  ...
  if (o) {
    ...
    b = a.options[a.selectedIndex].getAttribute("scope");
    if (!b) b = a.options[a.selectedIndex].text;
  ...
  if (r || o) {
    ...
    if (e) n = A;
    if (b != "") {
      c += "&s=" + encodeURIComponent(b);
      if (a.options[a.selectedIndex].value != "" && !e)
        n = a.options[a.selectedIndex].value;
    }
    if (f != "") c += "&cs=" + encodeURIComponent(f);
    if (g != "") c += "&u=" + encodeURIComponent(g);
  }
...
function getParentByNodeName(a, b) {
  ULShpi:;
  if (a != null && a != undefined)
    if (a.nodeType == 1 && a.nodeName == b) return a;
    else return getParentByNodeName(a.parentNode, b);
  return null;
}
function SearchNavigate(b) {
  ULShpi:;
  if (b == null) return;
  try {
    var a = getParentByNodeName(b.target, "A");
    if (
      a != null &&
      a.id != null &&
      (/^SRB_/.exec(a.id) || /^BBR_/.exec(a.id))
    ) {
      var d = a.href;
      if (
        d != null &&
        ...
        location.hostname.toLowerCase() == a.hostname.toLowerCase()
      ) {
        var e = d.lastIndexOf(".");
        if (e > -1)
          if (d.substring(e + 1, e + 4).toLowerCase() == "pdf")
            if (window.ActiveXObject) {
              ...
              try {
                ...
                if (c != null) {
                  ...
                  c.send(null);
                  if (c.readyState == 4)
                    if (c.getResponseHeader("X-Download-Options") != "noopen")
                      g = true;
                }
              } catch (f) {}
              var h = null;
              if (g)
                try {
                  h = new ActiveXObject("AcroPDF.PDF");
                } catch (f) {}
              if (h == null) {
                var i = new ActiveXObject("PdfFile.OpenDocuments");
                if (i != null)
                  if (i.ViewDocument2(window, d)) {
                    b.returnValue = false;
...
/*
page: http://www.greatamericaninsurancegroup.com/
url: http://www.greatamericaninsurancegroup.com/_layouts/search.js?rev=KH4kEGeuu3Mv6BpWoFYd%2FA%3D%3D
*/
