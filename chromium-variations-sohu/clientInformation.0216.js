...
(function (m) {
  var m = m;
  var d = m.using("$baseName.Utility");
  var q = m.using("$baseName.Business");
  var n;
  var b = "f";
  var l = "float";
  var t = "SOGOU_STAR_SETJSONADSLOT";
  var p = null;
  var a = null;
  ...
  var i = function () {
    if (f() == 1) {
      ...
      var u = v(document.referrer);
      if (u.hostname.lastIndexOf(r(document.location.hostname)) == -1) {
        return 1;
      } else {
  ...
  var o = function (y, x, A, w, u, v, z) {
    return function () {
      var D = document.getElementById(w);
      var B = document.getElementById(w + "_close");
      if (document.documentElement.clientHeight == 0) {
        bs_w = document.body.clientWidth;
        bs_h = document.body.clientHeight;
      } else {
        bs_w = document.documentElement.clientWidth;
        bs_h = document.documentElement.clientHeight;
      }
      if (!z) {
        var C = h();
        ...
        if (y == 2) {
          ...
          if (y == 3) {
            D.style.left = bs_w - v - A + "px";
          }
        }
        if (x == 1) {
          D.style.top = C + (bs_h - u) / 2 + "px";
        } else {
          if (x == 2) {
            D.style.top = C + "px";
          } else {
            if (x == 3) {
              D.style.top = C + bs_h - u + "px";
            }
          }
        }
      } else {
  ...
  var s = function (z, y, w) {
    var x = document.createElement("div");
    var A = document.createElement("div");
    var v = document.createElement("div");
    x.id = w.wrapperId;
    x.style.height = w.iframeHeight + y + "px";
    v.style.height = w.iframeHeight + "px";
    A.innerHTML = "¹Ø±Õ";
    d.bind(A, "click", function () {
      x.style.display = "none";
    });
    A.style.background = "#AFAFAF";
    A.style.textAlign = "center";
    A.style.width = w.iframeWidth + "px";
    A.style.height = w.close_height + "px";
    A.style.lineHeight = w.close_height + "px";
    A.style.fontSize = "12px";
    A.style.fontWeight = "800";
    A.style.color = "#FFFFFF";
    A.style.cursor = "pointer";
    var u =
      '<iframe id="{iframeId}" onload="document.sogou_ads{bi}_loaded=true;" src="{starServiceUrl}?{paramString}" width="{iframeWidth}" height="{iframeHeight}" align="center,center" marginwidth="0"  marginheight="0" scrolling="no" frameborder="0" allowtransparency="true" ></iframe>';
    v.innerHTML = d.template(u, w);
    if (z == 1) {
      x.appendChild(v);
      x.appendChild(A);
    } else {
  ...
  var k = function (F, L, u) {
    var P = {};
    var Q = {};
    q.randomArray.push(Math.random() * 1000000);
    P = q.parseSlotDataFromStar(F);
    if (F == null) {
    ...
    if (C == 1) {
      for (var K = 0; K < 2; K++) {
        var H = q.getAdsDomId(Q.get("bi"));
        var v = q.getAdsWrapperDomId(Q.get("bi"));
        var B = q.Param.serialize(Q);
        B += "&tmp_cdif=" + String(i());
        B += "&m=" + P.m;
        B += "&ex=" + P.ex;
        B += "&pp=" + P.pp;
        ...
        var E = s(O, I, y);
        if (d.browser.ie && (d.browser.ie == 6 || d.browser.ie == 7)) {
          document.body.insertBefore(E, document.body.childNodes[K]);
        } else {
          document.body.appendChild(E);
        }
        var G = o(K + 2, J, I, v, D + N, M, z);
        G();
        if (!z) {
          d.bind(window, "scroll", G);
        }
        d.bind(window, "resize", G);
        var A = q.Param.snap(Q);
        q.adsArray.push({ domId: H, uiParamSnap: A, win: window, js: b });
  ...
  if (b === "f") {
    ...
    var g = d.getPara(j);
    q.isAsyn = g.asyn;
    p = Number(window.sogou_ad_width);
    a = Number(window.sogou_ad_height);
    if (window.sogou_ad_id) {
      q.getSlotDataFromStar(sogou_ad_id, t);
    }
  }
})(window[___sogouNamespaceName]);
(function (e, h) {
...
/*
page: http://www.leishiwoliao.com/
url: http://images.sohu.com/cs/jsfile/js/f.js
*/
